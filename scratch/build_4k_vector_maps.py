...
def render_ultra_hd_map(src_path, dest_path, scale_factor=3):
    print(f"Processing {src_path} with {scale_factor}x Ultra-HD Super-Resolution...")
    ...
    # B) Text, Numbers, and Fine Symbols (Dark in original)
    text_mask_orig = (gray < 85) & (a > 30)

    # C) Corridors / Walkways (open pedestrian areas)
    corridor_mask_orig = (gray >= 165) & (gray <= 238) & (S < 35) & (~bg_mask_orig) & (a > 30)

    # D) Cyan Island Badges
    cyan_badge_mask_orig = (H >= 80) & (H <= 115) & (S > 35) & (V > 70) & (a > 30)

    # E) Store Footprints
    store_mask_orig = (~bg_mask_orig) & (~corridor_mask_orig) & (~text_mask_orig) & (a > 30)

    # F) Anchor Stores
    anchor_mask_orig = store_mask_orig & (S > 35)

    # 3. High-Definition Target Canvas (e.g. 4608 x 2154 for 1536x718)
    h_hd = int(h_orig * scale_factor)
    w_hd = int(w_orig * scale_factor)

    # Upscale masks using Lanczos4 and smooth thresholding for anti-aliasing
    ...
    print("Upscaling masks with Lanczos4...")
    ...
    cyan_hd = upscale_mask(cyan_badge_mask_orig, blur_radius=3)
    text_hd = upscale_mask(text_mask_orig, blur_radius=1)

    # 4. Extract Vector Contours of Store Walls and Building Footprints
    print("Extracting and vectorizing building contours...")
    # Clean binary mask of all store units for contour vectorization
    ...
    # Also detect sharp internal partition walls
    canny_edges = cv2.Canny(gray, 35, 105)
    # Exclude exterior background from edges
    ...
    # Text / Numbers: #f8fafc -> (252, 250, 248)
    # Cyan Badges: #0284c7 -> (199, 132, 2)
    ...
    for cnt in contours:
        # Simplify slightly to remove raster stairs while preserving true corners
        approx = cv2.approxPolyDP(cnt, epsilon=0.7, closed=True)
        # Scale contour points to HD resolution
        cnt_hd = (approx.astype(np.float32) * scale_factor).astype(np.int32)
        # Draw anti-aliased outline (thickness 3 px for 3x scale)
    ...
    edge_mask = canny_hd > 50
    # Soft blend edges
    for t in range(2):
        hd_canvas_u8[edge_mask] = [105, 80, 58]  # Crisp titanium slate wall border

    # 7. Draw Cyan Island Badges
    cyan_mask = cyan_hd > 80
    hd_canvas_u8[cyan_mask] = [185, 120, 10]

    # 8. Draw Luminous High-Contrast Numbers, Text, and Logos
    # Upscale text sharply with morphology
    text_sharp = text_hd > 90
    # Draw crisp anti-aliased white text
    hd_canvas_u8[text_sharp] = [252, 250, 248]

    # Re-enforce exterior background
    bg_sharp = bg_hd > 120
    hd_canvas_u8[bg_sharp] = [22, 13, 8]

    # 9. Soft Unsharp Mask for maximum Vector clarity
    gaussian = cv2.GaussianBlur(hd_canvas_u8, (0, 0), 1.2)
    unsharp_hd = cv2.addWeighted(hd_canvas_u8, 1.25, gaussian, -0.25, 0)
    unsharp_hd[bg_sharp] = [22, 13, 8]
    unsharp_hd[text_sharp] = [254, 252, 250]
    ...
    print(f"Generated Ultra-HD Map: {dest_path} ({w_hd}x{h_hd})")

for f in ['planta-baja.png', 'planta-uno.png', 'planta-dos.png']:
    name_dark = f.replace('.png', '-dark.png')
    render_ultra_hd_map(f, name_dark, scale_factor=3)
